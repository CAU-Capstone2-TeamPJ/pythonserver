...
1. 주소가 불분명한 항목은 제거합니다.
   - '주소가 불분명하다'의 기준은 다음과 같습니다:
     - 주소 문자열에 '로' 또는 '길'이라는 단어가 포함되지 않거나,
     - 주소에 숫자가 없는 경우,
     - 또는 '동'으로 끝나는 경우 (예: '서울 성북구 삼선동' 등은 특정 건물 위치 없이 지역 단위만 표현한 주소이므로 불명확합니다).
     예: '서울 성북구' → 불분명 / '서울 성북구 삼선동' → 불분명 / '서울 마포구 손기정로 32' → 명확함
2. 결과는 JSON 리스트 형식으로 출력해주세요.
